...
- The language module also has a two level structure. The lowest level is in terms of a predictive processing pipeline built on a sequence of discrete phonemes (for sound) or individual characters (for reading). This level incorporates an inference-prediction-generation modules whose job is to predict the next phoneme or character. The latent representation from this level is sampled at certain discrete instants that contain representations for whole words, and these are fed as input into a word level predictive processing pipeline at level 2.
The next word prediction done at this level is sent to the central ATL hub where it gets integrated with representations coming from other modalities. It is then sent back to the level 2 hub
to do the next word generation. Hence unlike in an LLM, next word generation is not based solely on word occurrence statistics, but on data from other modalities that are impinging on the brain.
