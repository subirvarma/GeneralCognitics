...
$$ Z = \sum_{[\sigma]} \exp[{\beta\over{2N}}\sum_{\mu=1}^p ]   $$
...
So the equilibrium states of the system (which are the same as the minima of  the average free energy ${\overline f}$) correspond to different values of the order parameter vector $M_{sp} = ( {m^1}_{sp},...,{m^p}_{sp} )$. 
