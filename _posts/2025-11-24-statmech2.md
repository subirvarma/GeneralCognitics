...
So the equilibrium states of the system (which are the same as the minima of  the average free energy ${\overline f}$) correspond to different values of the order parameter vector $M_{sp} = ( m^1_{sp},...,m^p_{sp} )$. But what is the relationship between this vector and the memories 
