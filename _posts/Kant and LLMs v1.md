...
- Science: These serve as models of reality, and constructed using mathematical models.
...
- Source 2 comes from our mind, which processes the information from Source 1 and generates a picture of the world as we perceive it, called the Phenomenon. However this picture does not reflect the Noumenon or base reality, but is a creation of our mind, and it does so with the main objective of helping us survive.

This idea does get a little getting used to when we first encounter it, and it is difficult to beleive that we are seeing around us is coming from within us, as opposed to existing out there. It requires a change in perspective which Kant called the Copernican Revolution in philosophy, since the idea that the Sun is stationary and that the Earth is going around is similarly difficult to grasp since it goes against our senses.
...
Einstein himself never reconciled to the idea that the models in physics were not describing a base reality, hence is oft quoted remark that "God does not play dice with the universe" when he was confronted by the indeterminism in the predictions of Quantum Theory. 
...
- There is an entity called the physical world which has its own objective reality independent of the mental world. Our perception is like a window on to the physical world and reflects a true picture of what exists in nature.
...
This raises the question: What is the rightmodel for the mind?  How can a set of physical processes happening within the brain give rise to our mind and its subjective experiences? 
...
So far we have discussed the ability of ANNs to create images, but this is not enough. You may recall from Anil Seth's theory that the critical skills that our brains exhibit is the ability to predict the next 'frame' of our visual perception. Can ANNs do that? It turns out that the generative models that are used to cretae images can be extended so that they can generate short video clips. Once again there are Latent Vectors for video that map from the Latent Space to the space of video clips.
These clips are of high quality, but they often betray ignorance of reality, such as a glass object bouncing when dropped on the floor instead of shattering. 
...
The brain is constantly updatng it model of reality, based on the information it is getting at every instant. This process starts when we first open our eyes, to the time that we die. As a result of this, its model of reality become more accurate over time, and any disagreements with it are gradually trained away. In order to do this it is required that the agent (i.e. humans) carry the model around in their heads. This is clealy not the case for the most advanced ANNs that exist currently. They require huge datacenters with lots of GPUs and memory, and have also have massive power requirements. Hence the way things work today, they are trained once using a large dataset, and after this all their predictions are based on the model that developed during the training process. Hence their world model is static, while our world model is dynamic. In order to get closer to the brain, ANNs should be able to contantly re-train themselves based on current data. This requires the following:

- The ANN will have to be given perceptual extensions, such as a camera and a microphone so that it can register new data. Obviously this cannot be done if the ANN is housed in data center, and there are two ways in which we can solve this problem: (1) Allow the ANN to go out into the world, as part of a robot. This clearly will not work with the current server based paradign, (2) Attach a camera and miscrophone to a human who is constantly feeding this information back to the ANN (this is a bit like the training that self driving cars do, where they drive on city roads for several years to improve their model).
- Alternatively the static training data that is used to do the initial training can be filtered so that all the training clips strictly reflect reality.

Why is it important for the ANN to be a able to predict a true picture reality? Other than the fact that it enables us to generate video clips that look like the real world, this ability is critical in doing planning. We will talk more about in the following sections, but here is a preview: Planning is the ability to be able to run future scenarios 'in our head', so that we can figure out the best wayof accomplishing a task. Hence in this case the generated images are no longer being corrected by real time data, so that it is important to get the generation right. If the ANNs ability to do this is suspect, then this will clearly impede it ability to do planning.
...
Fig. 16 shows the process by which a piece of text is converted into an image: We start with the text, and use an LLM to convert it into its Latent Vector. We then feed the Latent Vector into an image generating ANN to get the corresponding image. Hence effectively both text and image are reduced to the same Latent Vector, which can be decoded to generate one or the other.
...
- Solving a problem that involves knowledge of the world: Consider the example of trying to assemble a piece of furniture using instructions that came with it. The actual assembly process involves taking Actions in the real world, but assume that before the assembly starts we mentally try figuring out the various steps involved. The latter is an example of planning, and in order to carry it out we need a to create a mental picture of how the furniture would look if as we carry out the assembly step. These mental pictures would then correspond to the State of the assembly decision tree.
From this example we can see that in order to plan out a System 2 task planning successfully, our mind needs to give us an accurate picture of the reality (or State) that would result if we were to carry out an Action. 

Another important skill needed in navigating the decision tree is the ability to prune it down to a manageable size, and this skill is needed whether we are acting in the real world or doing planning. This issue arises since potentially one can take a large number of Actions at any stage in the tree, which means the tree size can quickly balloon to a huge number of nodes. However our minds have the ability to intuitively pair down the number of Action choices to a mangeable number. This is a critical skill and is needed whether we are solving a math problem or planning a vacation.

If the ANN is embodied in a robot, it is constantly receiving feedback about its Actions from the camera and orher sensors it has, so we can see how it can potentially solve a System 2 problem without any generative ability. However if we think more deeply ablout this, before the ANN can start taking any Actions, it has to plan out the sequence of ACtions that will increase the chance that it accomplishes its task and this has to be done even before it takes the next Action in the real world. Hence if we can't be sure of its Planning abilities due to hallucinations, then it won't be very useful.
...
- ANNs cannot be used to solve planning problems that involve the generation of accurate models of the real world, since with the current state of the art, the ANN has no means of knowing whether the next State it is generating corresponds to reality or not.
Problems in physics or even the solution of planning problems we encounter in our daily lives belong to this class.

How do humans have a more accurate model of the world as compared to ANNs? There are two main reasons:

- The world model in humans is developed during the process of childhood and growing up into an adult (in addition to some *a priori* concepts that come pre-loaded into our brains, as Kant pointed out). During this time the data we get data directly from the world around us. On the other hand ANNs are trained using data (which can be video, images or language) which may or may not correspond to reality. Hence ANNs have a harder time obtaining an accurate world model from their training data.
- The human world model is being constantly updated during our waking hours, all throughtout our lives. This is done using the process that Anil Seth has proposed, whereby our brain is contantly generating predictions using it current model, and these are verifies against the data that is coming through our senses. If there is a difference between the two, then our internal model is modified in order to get a better prediction. ANNs on the other hand don't have any kind of interaction with the real world. Once they get trained, then all their predictions are generated by their stored model, and they have no opportunity to make improvements.

What can we do to improve ANN performance for class 2 problems involving the real world? There are potentially two ways:
...
- Use an architecture similar to the one that Anil Seth proposed for the brain, i.e., after the intial training ic complete, constantly check the predictions with the new data that is coming in, and use the difference to fine tune the ANN.

The latter way requires that the ANN be localized to the Agent, which could be a robot, and thus the fine tuning will be customized to the sensory data coming to that particular Agent. This is not feasible today since the most advanced models require huge amounts of compute and memory that is only feasible in the Cloud (unless the provider partitions the available compute resources into individual chunks, similar to the VM concept). The fine tuning will take time since it will be a gradual process as the Agent acts out in the real world and gathers more data. Hence this would be a longer term process in the Agent gradually becomes more expert in the task as its world model improves over time. Note that the improvement in the world model happens during interactions with the real world, but the Agent has a better and updated world model, it can also do a much better job in Planning for class 2 problems. Hence in some sense Seth's model for the brain is a requirement, since the process of Panning for the future requires a good Generator. This requirement by itself points out the main weakness of the "perception as a window into reality" model, since it doesn't involve any generation.

What does all this have to do LLMs, since we have been talking about image generation for all of this section? Well, given the isomorphism between language and images, the whole argument can be cast in terms of langauge, and it will remain equally valid. Clearly in case of mathematics there are no images involved (though there are mathematicians who claim that their creativity is due the ability to visualize the math they are doing), hence the navigation of the decision tree is entirelly through symbols. For physics and other sciences which involve interaction with the real world, the decision tree can be navigated using purely language, since as Wittegenstein (and Latent Spaces) have taught us, they are equivalent. 

The ability to prune the decision tree is key to intelligent behavior, and in the human realm it distinguishes the most skillful people from the rest of us. How are ANNs able to prune decision trees? It turns out that this ability is actually System 1 skill, even though it being performed within the context of a System 2 task. In other words the ability to figure which course of Action is the best is something that we do intuitively and sometimes without knowing why we made a particular choice. Since ANNs are good at performing System 1 tasks, it stands to reason that they can prune decision trees quite well too. One of the smartest ANN systems is the one called AlphaGo that was deisgned by DeepMind to play Go, Chess and other board games. AlphaGo consists of a Decision Tree (that is trained using Reinforcement Learning) and an ANN, whose main function is to prune the game's decision tree. In the case of games, the world model is relatively simple since it is confined to the board, however similar designs are being pursued to do planning using ANNs in real world contexts too.

The best scientists among us have an extreme ability in the decision tree pruning area. Indeed Einstein was able to deduce the Theory Special Relativity from the single fact that the speed of light is an universal constant and the General Theory from the Equivalence Principle. It is unlikely that ANNs will get to this level anytime soon, however ANNs do have another scientific ability that will turn out to be as useful: This is the ability to recognize patterns in a mass of data, which are not visible to humans. The ability to form a model for language is a good example of this. There are problems in physics, and even more so in other sciences such as Biology or Economics, where there is a lot of input-output data, but no simple formula that connects the two . However ANNs are capable of coming up with a mathematical mode for these cases too, the only difference is that now the model is not in the form of a few elegant equations, but is captured in the hundreds of millions of parameters of the ANN. Mathematics at the most advanced level also involvesdecision tree pruning, especialli in trying to come up with new mathematical structures. Great mathematicians are able to come up new structures that result in fruitful developments, and the ability to do so cannot be captures by logic alone.
...
So we finally come to topic that was advertised in the title of this essay: What would Kant think of LLMs (and ANNs in general)?
I think Kant would be very pleased with LLMs, since they provide a further validation of his fundamental idea that the brain is essentially a reality generating machine (with the help of new data coming in from the Noumenon and some *a priori* concepts that we are born with)? Lets see how LLMs and ANNs have similar properties.

![](https://subirvarma.github.io/GeneralCognitics/images/Kant28.png) 

Figure 19

The top part of Fig. 19 shows the process of text generation using the LLM called Transformer. The input prompt into the model is shown on bottom left, while the generated text is shown in the upper right. The input text is converted into a Latent Vector representation which is then used by the generation process. The generation happens in a recursive manner on a word by word basis. This process can be further abstracted as shown in the figure in the bottom part of Fig. 19, and this has a direst correspondence to the operation of brain in Anil Seth's model as shown in Fig. 5: The input prompt can be considered to be analogous to the sensory signals coming in from the Noumenon. This information is mapped by the LLM into the Language Latent Vector Space by the Encoder part of the LLM. The Decoder part of the LLM then takes the information in the Latent Vector and converts into another piece of text. In some sense the LLM is taking a piece of information that is incomplete or lacking detail, and then proceeds to supply the missing parts as part of the generated text. The same process happens in the case of images as shown next.
...
This is exactly the mechanism that Kant proposed for perception, with the rough information now coming in from the Noumenon, which is then converted into the multi-colored high definition image of the world that we see in front of us. 

The only difference between ANNs and the brain is that ANNs are trained one-time and then put to work making predictions, while the brain undergoes continuous training, as shown in Anil Seth's model in Fig. 5. At the present time it is not practical to continuously re-train (or fine tune) ANNs, since a single ANN is housed in an expensive datacenter and serves a large number of users. Hence we cannot change the ANN parameters based on aany one users input prompts. In order to make the ANN more brainlike, we will have to personalize them to individual users, so that ANN can be contiuously finetuned based on the prompts that the user is feeding into it. The recently announced ANN on the Apple iPhone is a staep in that direction, though the local ANNs are much smaller and less powerful than the best server based models out there. 

How about other aspects of Kant's system, are they also reflected in ANNs?
