...
The process of image generation by an ANN is shown in Fig. 20. It takes in information that is rough or incomplete, susch as a sketch of a human head, encodes it into a Latent Vector, and then maps it to another Latent Vector that represents a completed image with all the details, which is then decoded.

Hence both LLMs and image ANNs have the propert that they take in incomplete or rough information and then supply the missing details as part of the generation process. This is exactly the mechanism that Kant proposed for perception, with the rough information now coming in from the Noumenon, which is then converted into the multi-colored high definition image of the world that we see in front of us. The only difference between ANNs and the brain is that ANNs are trained one-time and then put to work making predictions, while the brain undergoes continuous training, as shown in Anil Seth's model in Fig. 5. At the present time it is not practical to continuously re-train (or fine tune) ANNs, since a single ANN is housed in an expensive datacenter and serves a large number of users. Hence we cannot change the ANN parameters based on aany one users input prompts. In order to make the ANN more brainlike, we will have to personalize them to individual users, so that ANN can be contiuously finetuned based on the prompts that the user is feeding into it. The recently announced ANN on the Apple iPhone is a staep in that direction, though the local ANNs are much smaller and less powerful than the best server based models out there. 
