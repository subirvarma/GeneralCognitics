...
- Solving a problem that involves knowledge of the world: Consider the example of trying to assemble a piece of furniture using instructions that came with it. When we actually assemble the piece then of course that corresponds to taking Actions in the real world, but assume that before the assembly starts we are mentally figuring out the various steps involved. The latter is an example of Planning, and in order to carry it out we need a to create a mental picture of how the furniture would look if we carried out an assembly step which would then correspond to the State.

From the last example we can see that in order to plan out a System 2 task successfully, our mind needs to give us an accurate picture of the reality (or State) that would result if we were to carry out a particular Action. Another important skill needed in navigating the decision tree is the ability to prune it down to a manageable size. This issue arises since potentially one can take a large number of Actions at any stage in the tree, which means the tree size can quickly ballon to a huge number of nodes. However our minds have the ability to intuitively pair down the number of choices to a mangeable number. This is a critical skill and is needed whether we are solving a math problem or planning a vacation.

Can we replace our mind with an ANN and still be able to solve System 2 type problems?
...
Planning is an important activity and even if the ANN is controlling a robot that is taking Actions in the real world, the ANN has to plan out all it Actions to make sure that they achieve their end objective (or even a few steps in advance), and this has to be done even before it takes the next Action in the real world. Hence if we can't be sure os its Planning abilities due to hallucinations, then it won't be very useful.

How do humans have a more accurate model of the world as compared to ANNs? There are two main reasons:

- The world model in humans is developed during the process of childhood and growing up into an adult (in addition to some *a priori* concepts that come pre-loaded into our brains, as Kant pointed out). During this time the data we get data directly from the world around us. On the other hand ANNs are trained using data (which can be video, images or language) which may or may not correspond to reality. Hence ANNs have a harder time obtaning an accurate world model from their treaining data.
- The human world is being constantly updated during our waking hours, all throughtout our lives. This is done using the process that Anil Seth has proposed, whereby our brain is contantly generating predictions using it current model, and these are verifies against the data that is coming through our senses. If there is a diffrence between the two, then our internal model is modified in order to get a better prediction. ANNs on the other hand don't have any kind of interaction with the real world. Once they get trained, then all their predictions are generated by their stored model, and they have no opportunity to make improvements.

What can we do to improve ANN performance for class 2 problems involving the real world? There are potentially two ways:

- Make sure that the training data only consists of samples that reflect reality, i.e., confine it to the smaller cisrcles shown in Fig. 17. However this is not the ideal solution since the world is a dynamic place, hence the world model that the ANN learns today will be out dated by tomorrow.
...
What does all this have to do LLMs, since we have been talking about image generation for all of this section? Well, given the isometry between language and images, the whole argument can be cast in terms of langauge, and it will remain equally valid.

- ANNs role in pruning large Decision Trees.
