...
The most insidious cause of lower signal to noise in single carrier QAM systems is due to a wireless specific problem called multipath self interference, and it turns out that an increase the bandwidth B makes it worse. Also increasing the power S does not help much either, since the interfering signal also becomes stronger thus causing the ${S\over N}$ ratio to decrease!
