...
This is the province of text to image models which turn a textual description into the corresponding image. Image to image models or text to text models (also called LLMs) also fall into this class, hence this constitutes a very important class of AI models. These models can be understood as minimzation of the energy function $E(x_1,...,x_N, c)$ where the new variable $c$ represents a latent vector that represents the textual description. This energy function is modeled by the neural network $E_{\theta}(x_1,...,x_N, c)$, which is trained on the data set $(x_1,...,x_N,c)(i), i=1,...,M$. During inference samples are generated by starting from a random vector, and then using the multistage sampling process to find the configuration $(x_1,..,x_N)$ that has the minimum energy given the latent vector $c$. This is actually implenented using the model ${\partial E_{\theta}(x_1,...,x_N,c)\over{\partial x_i}}$ for the score function, as we will explain. Each calue of $c$ can led to multiple possible generated configurations, depending upon our choice of the initial random vector.
