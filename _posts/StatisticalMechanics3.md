...
The right hand side of the figure shows an EBM model with energy function $E_W(x_1,...,x_N)$, with corresponding Boltzmann distribution $p_W(x_1,...,x_N)={e^{-E(_Wx_1,...,x_N)}\over{Z_W}}$. The model is considered to be trained if we sample from $p_W(x_1,...,x_N)$ and the resulting image is similar to those in the training dataset, and there are two ways of doing this:
