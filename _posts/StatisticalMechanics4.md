...
Thus the raw sensory data is never directly experienced and what we do experience is modulated by our prior experiences and our expectations.
...
Hence perception is the process of combining the brain's internal model of world, with information that is coming through our senses. But where does the internal model come from? It is created by a continual process of learning that our brains are engaged in, from the time we first open our eyes. 
...
Central to the description of the Predictive Processing model is a probability distributions of the form $p(x_{n+1}|x_{\le n}, a_n)$ which captures the evolution of the brain's internal state $x_n$ as a function of the actions that the organism is taking. These is referred to as a **world model**, since it serves as the organism's internal model of how the world evolves as it takes various actions. 
...
## Models for Prediction Using EBM/Diffusions

The  Latent State based Predictive Processing LPP framework tells us that both perception and planning abilities in living organisms is crtically dependent on their ability to build a world model, as captured by the conditional probability distribution $p(x_{n+1}|x_n, a_n)$ and in this section I will show how EBMs can be used to model this distribution.
