...
## A Model for Predictive Processing Using EBMs
...
We have approached the process of generation using the language of annealing based energy minimization. However these types of algorithms are more commonly referred to as de-noising algorithms, and the reason for this can be seen in the figure. We start off with a random state $z(T)$ which looks like pure noise, and then gradually as the optimization progresses, a more legible image emerges. The image become sharper in the later stages of the optimization as shown. The de-noising perspective can be formalized to create an alternative method of sampling from a distribution, called Denoising Diffusion Probabilistic Models or [DDPM](https://arxiv.org/abs/2006.11239).
...
Another aspect on the biological plausibility issue is the training process. The model clearly has a number of training algorithms at its disposal based on back propagation or backprop such as the score function technique. However evidence for backpropagation has not been found in brains. The training process in brains involves changes in the synapse strengths using local mechanisms such as the Hebb's rule. 
EBMs can be trained in a similar local fashion by using maximum likelihood based training, and this results in weight updates that use only local information. 
It has been shown by Hinton that the application of maximujm likelihood training to the Boltzmann machine, which is a smpler EBM model, results in weight updates that are strictly local and can be understood to be identical to Hebb's rule. In Part 3 I described a training procedure for diffusion EBMs also based on maximum likelihood that can be implemented by using sampling, which makes it biologically plausible.
...
### Some Comments on the Connectome Interconnection Architecture

![](https://subirvarma.github.io/GeneralCognitics/images/stat95.png) 

Figure 4: Variation of energy functions depending on conditioning

What are the implications of having a complex energy function on the micro-architecture of the brain, i.e., the connectome?
Some insight into the problem can be obtained by looking at the development of Hopfield Networks. As described in [Part 2](https://subirvarma.github.io/GeneralCognitics/2025/11/24/statmech2.html), Hopfield networks were designed to serve as associative memories rather than for capturing the distribution of a training dataset as in Boltzmann machines. However it works using similar principles of trying to mnimize energy functions, so its design is relevant here. The initial design for the Hopfield network involved pairwise interactions between nodes in a fully connected spin glass type network, and in shown on the left hand side of the above figure. This design was biologically plausible since neuron interactions in the breain are known to be of the two-node type.
...
With our EBM/diffusion models we are in a similar situation as the new Hopfield network, since clearly the complex energy functions that come out of training involve interctions between possible a large number of nodes if laid out in the form of an inter-connection circuit diagram. However Hopfield and Krotov showed that a modified network consisting of a mixture of visible and hidden neurons could be found such that the energy function for the visible part coincided with a complex non-linear function. Moreover, all connections between nodes in this network were of the two-node type, which made it biologically pluasible. This discovery also lay behind the power of the Boltzmann machine, as was discovered by Hinton mre than a decade earlier. Boltzmann machines also featured hidden nodes, and Hinton showed that this resulted in an increase in the number of possible data distributions that the visible nodes could represent.

![](https://subirvarma.github.io/GeneralCognitics/images/stat96.png) 

Figure 4: Variation of energy functions depending on conditioning

Based on this insight, a way to make EBM/diffusion models biologically plausible is by creating an equivalent network which has both perception and hidden nodes, such the energy function of the perception neurons is the same as given the model. There is a rich interconnection of the perception and hidden nodes among themsevles and between groups, but all interactions are of the two node type. This results in a design of the type shown in the above figure, in which the brain has masses of hidden neurons interacting with the perception neurons. There is actually a model for the brain that was proposed by Karl Friston that involved both visible and hidden neurons, and this is described in the next section.
But the question arises whether hidden neurons are strictly necessary from the modeling point of view, or EBM/diffusion models without hidden neurons are sufficient to mimic the brain.
I will consider this issue after discussing the Friston model.
...
### An EBM Model for Active Inference
