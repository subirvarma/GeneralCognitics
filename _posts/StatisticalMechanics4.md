...
However unlike Predictive Processing, Active Inference is a based on a mathematical framework that uses Bayes Rule, and the principle of minimization of variational free energy. 
From the machine learning point of view, this model is similar to Helmhotz Machine that was proposed in the mid-90s by Hinton and others. A more recent version along the same theoretical lines has resulted in an image generation model called VAE or Variational Auto-Encoder.
...
The tea making scenario also points towards a way in which planning cann be combined taking actions in the real world. The above figure shows a way by which an 
