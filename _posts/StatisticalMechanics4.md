...
In Part 3 I looked at diffusion models in AI and how these could be regarded as more advanced versions of the Boltzmann Machine. Diffusion models are also based on the idea of an energy function, but now it is a complex creature that is modeled by artificial neural networks with hundreds of millions of parameters. In the process we moved past the idea of modeling the details of the interactions between the system nodes. Instead the system dynamics are be captured by changes in the value of the energy function as it settles into an equilibrium state.
However unlike the Boltzmann machine, diffusion models don't require the incorporation of hidden states, since the use modern artificial neural networks such as transformers as function approximators is sufficient to capure the complexity of the energy function.
...
Hence generative AI systems are imitating the energy function of the brain, not its micro connection architecture.  But the fact that they work so well shows that a model at the level of energy functions is sufficient to create an artificial brain and powerful training algorithms such as backprop.
...
This is a qualitative framework of how the brain works and its main proposal is that the brain's model is predictive. Hence the brain is not just modeling the state of the world as it exists at any instant, but does prediction, i.e., it forecasts what the world might look like in the future. 
...
Within the Predictive Processing framework, models can be built with or without the use of internal states in the brain, also called hidden or belief states, and we will consider both types.
The most prominent of the models incorporating hidden states is called the Active Inference framework.
Active Inference models the internal states in the brain by using a
mathematical framework based on Bayes Rule, and the principle of minimization of variational free energy. This framework splits up the process of perception into three parts, namely inference of the internal state based on information coming from the senses, followed by prediction of the next state and then finally generation of the next state. Inference results in a latent state representation that the brain uses internally for making the next prediction.
...
Part (a) of figure 1 shows a model for perception, using the three operations of inference, prediction and generation. I will show that all of these three modules can be modeled using the principle of energy minimization. Input sensory data is converted into a latent space representation using the inference module, and this is converted into a system state by combining it with the previous state.
The prediction module then does its job using this representation. Finally the generation module converts the latent space representation back into pixel space.
Part (b) of the figure another system in which the internal state is not explicitly modeled, the model converts input pixels directly into output pixels, while also making use of previous predictions. This system can also be modeled by an EBM which implicitly handles all the three operations of inference, prediction and generation.
Model (b) use Langevin sampling to do the energy minimization, and this results in a parallel prediction/generation of all pixels. However it turns out that model (b) can also be implemented by doing pixel-by-pixel generation and this is shown in figure (c). This is implemented using auto-regressive models such as the transformer, and it can be shown that the pixel-by-pixel generation can also regarded as a type of energy minimization. 

If we follow the chain from (a) to (b) to (c), it follows that process of perception in the brain can be modeled using auto-regressive models. However the mechanics of pixel-by-pixel generation is very different than the full scale inference/prediction/generation pipeline in part (a). Biological brains probably implement these operations using the system shown in part (a), while we have discovered an equivalent way of doing so by using auto-regressive models. 
However we should not be looking for transformer equivalents in the brain. If we are able to show that the transformer is equivalent to a system of interacting nodes working according to the principle of energy minimization, then this problem becomes moot, and the main purpose of this article is to show that this is indeed the case.
...
Hence perception is the process of combining the brain's internal model of world, as captured by the conditional distribution $p(y_{n+1}|y_{\le n})$, with information new $s_n$ that is coming through our senses. But where does the internal model $p(y_{n+1}|y_{\le n})$ come from? It is created by a continual process of lesrning that our brains are engaged in, from the time we first open our eyes. 
...
Part (b) of the figure shows a sequence of possible rollouts of the perception-action planning sequence, from a starting state $s_1$. An individual rollout is one path through this graph, and an example is shaded in red. A rollout terminates when the organism reaches the end state labeled T. 
...
This sort of daily planning that we do in our lives is considerably simpler than the sophisticated MCTS type tree searches used in Alpha Zero. Morever we have the addiitonal advantage of accumulated knowledge that we bring to bear in figuring out what action to take.
...
The  Direct Predictive Processing or DPP framework tells us that both perception and planning abilities in living organisms is crtically dependent on their ability to build a world model, as captured by the conditional probability distribution $p(y_{n+1}|y_{\le n}, a_n)$. 
...
I am hypothesizing that there is a set of neurons $(x_1,...,x_N)$ in the brain whose current equilibrium state results in the image that we see (in case of vision) or a thought we have (in case of language).
...
A single step of sampling for the $t^{th}$ step of the optimisation is done using the equation
