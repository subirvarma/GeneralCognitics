...
Note that we can make the successive points as close together as we like, since space and time are both continuous. It turns out, that this is as close as we can get to picturing movement. Note that movement is actually happening over a continuous interval but our brains have a great deal of difficulty picturing this. We are more comfortable with points in space, and starting from a point we look for the next point in space that the arrow will occupy. However math is telling us that there is no next point! But this does not mean that movement cannot happen. The arrow does move, but its moving over a dense continuum of points which we can't visualize very well.

Is the rigorous definition of the derivative and the attendant ideas of limit and continuity all that there is to resolving the paradox? Not quite, since it raised further questions about the nature of the continuum. The idea of a continuum was introduced in order to resolve tha paradox, but what exactly is it? 
The answer to this question required a greater understanding of the concept of infinity.

## What is the Continuum?

It turns out that trying to pin down the nature of the continuum is another very difficult problem having to do with the concept of infinity, and one that was not solved until the latter half of the 19th century. The main contributor to this were the the German mathematicians Richard Dedekind and George Cantor. Dedekind gave a rigorous proof of the fact that there exist numbers that are neither integers nor fractions (or rational), but belong to category that he called Real Numbers. When expressed as a decimal, they have a never ending series of digits after the decimal point that are non-repeating. Irrational numbers such $sqrt{2}$ for example belong to this category. When Real Nunbers are taken into account, an interval no longer consists of a set of discrete points, but becomes a continuum. 
