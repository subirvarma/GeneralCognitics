...
## How Does the Arrow Move from Through Space

In the Newtonian/Differential Calculus picture of the arrow's flight, the arrow has a finite velocity at all point in time i.e., the arrow has a vertain velocity while at point A at time $t_A$, and this enables it to move to point B at a later time $t_B$. Since it is ""not in motion at both these time instants, when does it actually move from A to B?
We can try to pin down the arrow's trajectory by looking at intermediate instants of time, for example if $t_C = {t_A + t_B\over 2}$, then the arrow will be at the point $x(t_C)$ at $t=t_C$. But this not enough to pin down the motion, since we still don't know how the arrow got from $x(t_A)$ to $x(t_C)$. We can continue theis procedure of halving the time intervals *ad infinitum*, but however many times we do it, it is still impossible to pin down the instant at which the arrow is actually moving!
This description of the arrows flight given above implicitly assumes that both space are made up of discrete points, so that the arrow is moving from one discrete point to another as time progresses. This is clearly not a satisfactory picture, since given the starting point $x(t_A)$, clearly there is no 'next' point that the arrow can go to next, since if there was any such point, one can always find anothet point that is closer to the starting point. 

It turns out that trying to pin down when the motion actually happens is a another very difficult problem, and one that was not solved until the latter half of the 19th century. The main contributor to this were the the German mathematicians Richard Dedekind and George Cantor. Dedekind gave a rigorous proof of the fact that there exist numbers that are neither integers nor fractions, but belong to category that he called Real Numbers. When expressed as a decimal, they have a never ending series of digits after the decimal point. When Real Nunbers are taken into accouint, the number line no longer consists of a set of discrete point, be becomes what is called a continuum. 

George Cantor realized that the fundamental problem with the arrow paradox is the lack of understanding of the nature of infinity.
He made a number of important contributions to understanding the nature of the continuum, specifically he was able to prove the following:

- If cardinality of a set is defined as the number of elements in it, then obviously the cardinality of the set of Natural Numbers is infinity. Cantor gave this infinity a symbol $\aleph_0$. Cantor further showed that the cardinality of set of all Rational Numbers (those which can be expressed as fractions of Natural Numbers) is also $\aleph_0$.
- Then Cantor made the surprising discovery that the cardinality of the set of Real Numbers is not $\aleph_0$. In fact it is another type of infinity, whose cardinality he called $c$, and furthermore the two infinities can be compared, so that $c > \aleph_0$.
