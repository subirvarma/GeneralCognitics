...
It is difficult to visualize motion in a continuum, our brains are wired to think of motion as a discrete set of positions at discrete points in time. But these discrete points of time are
precidely the ones at which the object is being observed. This leaves the mystery of what exactly is the object doing in-between two such observations. If we go by Newton's Laws of Motion, then the position (and velovity) of the object can be predicted once we know its initial position, velocity and the forces acting on it. In particular in the absence of any force, an object moving at a particular velocity will keep moving at the same speed. This would allow us to track its position, even though it is not being observed.
...
Going back to Zeno's paradox, Quantum Mechanics tells us that a particle exists as what we understand to be a particle, only at the times at which it is being observed. Nobody has seen a particle while it is transitioning between two observations, since as soon as we observe it, it becomes a particle! This goes back to the conundrum we had raised in previous sections about trying to visualize the actual motion of a particle, as opposed to just saying that it is at spot A at time $t_A$ and at spot B at time $t_B$. Quantum Mechanics is telling us that this is as close as we can get in describing motion, since the particle exists as a diffuse wave like entity in-between two observations.

Another way of explaining the situation is as follows: A particle in itself does not possess  properties such as location or velocity. These properties only come into existence when the particle is being observed, i.e., interacting with an observer. When the particle is not being observed, its properties are un-knowable.
...
In the original paradox, Zeno concluded that the particle is not moving at all at any instant of time. Calculus told us that the particles does move, but any particular instant it is stationary and the only way to tell that it is moving is by observing it at two different instants. Quantum Mechanics also comes to the conclusion that the particle does move, though the movement is non-deterministic and does not obey Newton's Laws of motion. This raises the following interesting question: If the particle is not moving at the instant of observation, what if we were to observe with an extremely high frequencey, almost like continously observing it? Quantum Mechanics says that in this situation the particle won't change state, for example if it was stationary, then it will continue to be stationary and motionless (hence a state change is triggered by the act of non-observation). This is known as the Quantum Zeno Effect. If a classical particle were to be observed at an extremely high frequency, Newton's Laws tell us that it will be observed to move, but by infinitesmal amounts.
