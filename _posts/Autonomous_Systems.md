...
Li et.al. trained an 8-layer GPT model with an 8-head attention mechanism and a 512 dimensional hidden space, in an autoregressive fashion, which they called OthelloGPT. This is fed with a word embedding consisting of 60 vectors, one for each possible board position to get the sequence 
$x_t^0, 0\le t\le T-1$, where $T$ is the length of the sequence fed so far. The intermediate features for the $t^{th}$ token after the $l^{th}$ layer is given by $x_t^l$. Note that $x_t^8$ goes into a linear classifier to predict the logits for the next move.
