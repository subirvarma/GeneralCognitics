...
Boltzmann's great insight was the realization that this randomness could be captured in terms of a probability distribution, once the system is in equilibrium, under the following assumptions: 

- The system can exist in one of several microstates, such that the energy of the $i^{th}$ microstate is $E_i$.
- At fixed temperature $T$, the system moves randomly from one microstate to another under the influence of thermal motion,

Under these assumptions the equilibrium probability that the system is in the $i^{th}$ microstate is given by the Boltzmann distribution

$$ p_i = {\exp^{-\beta E_i}\over{\sum_j \exp^{-\beta E_j}}}  $$

where $\beta$ is the inverse temperature $\beta={1\over T}$. This is one of the most important formulae in all of physics and underlies statistical mechanics.

The biggest theoretical advance was a precise definition for the entropy of a collection of particles. Entropy had been introduced in the mid-1800s by Clausius as way of restating Carnot's results for the efficiency of the ideal steam engine, and it is something that could be measured macroscopically, but its true nature remained a mystery.
...
so that greater unceratinity about the levels led to higher entropy. For example $S$ is maximized at $S=\log n$ if $p_i = {1\over n}$ for all $i$, i.e., we have the least amount of information about the systems's state, and it minimized at $S=0$ if one of the $p_i$'s is one and the rest zero.
This implied that if we have a more detailed view of the system then the entropy would be lower since the uncertainity about system's state is lower.
...
- The electromagnetic radiation within a closed chamber can be considered to be a type of gas, but made up of photons rather than atoms, and its physics can be analyzed using the methods of statistical mechanics. This is precisely what Max Planck did in 1901, when he noticed that in order to get agreement with experimental data, energy levels have to come in discrete chunks, thus launching the science of quantum mechanics. In the 1920s Satyendra Nath Bose improved upon this model for radiation, which led to Bose-Einstein statistics in quantum theory.
